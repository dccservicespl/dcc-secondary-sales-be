...
import com.dcc.osheaapp.report.user.controller.UserExcelReportInputDto;
...
@RequestMapping( "/api/report/outlet")
public class OutletReportController {
    ...
    @Autowired
    OutletReportExportReqHandler
     handler;

    @Operation(
            summary = "Export outlet Excel Sheet",
            description = "Export outlet data in excel sheet ")
    @PostMapping("/ws")
    public Object exportWS(
            @RequestBody OutletExcelReportInputDto dto)
            throws IOException {
