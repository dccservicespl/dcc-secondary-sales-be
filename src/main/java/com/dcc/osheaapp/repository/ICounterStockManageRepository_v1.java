...
@Repository
public interface ICounterStockManageRepository_v1 extends JpaRepository<CounterStockManageVo, Long> {

	@Query(nativeQuery = true, value = "SELECT * FROM counter_stock_manage "
			+ "where DATE_FORMAT(updated_on,'%Y-%m-%d') = DATE_FORMAT(:updatedOn,'%Y-%m-%d') and outlet_id = :outletId and updated_by = :updatedBy ")
	List<CounterStockManageVo> findPocketMISByDateOutletUser(Long outletId, Date updatedOn, Long updatedBy);

	@Query(nativeQuery = true, value = "SELECT * FROM counter_stock_manage "
			+ "where DATE_FORMAT(updated_on,'%Y-%m') = DATE_FORMAT(:updatedOn,'%Y-%m') and outlet_id = :outletId ")
	...
	@Query(nativeQuery = true, value = "SELECT distinct outlet_id FROM counter_stock_manage "
			+ "where DATE_FORMAT(updated_on,'%Y-%m-%d') = DATE_FORMAT(:updatedOn,'%Y-%m-%d') ")
	...
	@Query(nativeQuery = true, value = "SELECT * FROM counter_stock_manage "
			...
			+ "and sub_category_name = :subCategoryName and DATE_FORMAT(created_on,'%Y-%c') = concat(YEAR(now()) , \"-\", MONTH(now()))")
	...
	// @Query(nativeQuery = true, value = "SELECT * FROM counter_stock_manage "
	// + "where DATE_FORMAT(updated_on,'%Y-%m') = :monYr and outlet_id = :outletId
	// ")
	// List<CounterStockManageVo> findMonthlyOutletStocks(Long outletId, String
	// monYr);
	...
	@Query(nativeQuery = true, value = "SELECT * FROM counter_stock_manage "
			+ "where DATE_FORMAT(updated_on,'%Y-%m') = :monYr and outlet_id = :outletId order by product_id")
	...
	@Query(nativeQuery = true, value = "SELECT * FROM counter_stock_manage "
			+ "where DATE_FORMAT(updated_on,'%Y-%m') = DATE_FORMAT(:updatedOn,'%Y-%m') and outlet_id = :outletId and product_id in :products ")
