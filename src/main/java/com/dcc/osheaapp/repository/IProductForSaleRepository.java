...
@Repository
public interface IProductForSaleRepository extends JpaRepository<ProductForSaleDto, Long> {
  ...
  @Query(
      ...
      value =
          "SELECT a.id ID, a.category_id categoryId, "
              ...
              + "a.product_ptr productPTR, a.product_vdo_link productVdoLink, a.size, a.unit, a.updated_by updatedBy, "
              + "a.updated_on updatedOn, a.product_code productCode,"
              + "b.closing_stock as availableStock, a.division, b.transaction_date transactionDate "
              + "FROM product a "
              + "inner join counter_stock_manage b on a.ID = b.product_id "
              + "and DATE_FORMAT(ifnull(b.transaction_date, b.created_on),'%Y-%m') = :monYr "
              //+ "DATE_FORMAT(b.transaction_date,'%Y-%c') = concat(YEAR(now()) , \"-\", MONTH(now())) "
  ...
  List<ProductForSaleDto> getProductForSale(Long categoryId, Long outletId, String monYr);
  ...
  @Query(
      ...
      value =
          "SELECT a.id ID, a.category_id categoryId, "
              ...
              + "a.division, b.transaction_date transactionDate "
              + "FROM product a "
              + "inner join counter_stock_manage b on a.ID = b.product_id "
              + "inner join product_category pc on pc.ID = a.category_id "
              + "left join product_category pc2 on pc.parent_id = pc2.ID "
  ...
  @Query(
      ...
      value =
          "SELECT a.id ID, a.category_id categoryId,   "
              ...
              + "a.product_code productCode,  "
              + "(b.sale - b.sale_return) as availableStock , a.division, '' as purchaseAmount, b.transaction_date transactionDate  "
              + "FROM product a  "
              ...
              + "and DATE_FORMAT(ifnull(b.transaction_date, b.created_on),'%Y-%m') = :monYr "
              + "group by a.id; ")
  List<ProductForSaleDto> getProductForSaleReturn(Long categoryId, Long outletId, String monYr); // , Long baId
}
